...
@registry.register_model("mini_gpt4_masker")
class MiniGPT4Masker(Blip2Base):
    """
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts):
        if prompts:
            p_before_embed_list, p_after_embed_list = [], []
            for prompt in prompts:
                ...
                p_before_embed_list.append(p_before_embed)
                p_after_embed_list.append(p_after_embed)

            p_before_embeds = torch.cat(p_before_embed_list, dim=0)
            p_after_embeds = torch.cat(p_after_embed_list, dim=0)
    ...
    def forward(self, samples):
        ...
        text = [t + self.end_sym for t in samples["question"]]
